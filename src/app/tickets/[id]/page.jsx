          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {[...Array(3)].map((_, i) => (
                <div
                  key={i}
                  className="h-16 bg-stone-200 dark:bg-neutral-800 rounded-xl"
                />
              ))}
            </div>
      ...
      {expired ? (
        ...
        <div className="grid grid-cols-4 gap-2 text-center">
          {[
            ...
            <div
              key={unit}
              className="bg-stone-50 dark:bg-neutral-800/50 rounded-xl py-2.5 border border-stone-100 dark:border-neutral-800/40"
            >
...
export default function TicketDetailsPage() {
    ...
    const fetchTicket = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API}/api/tickets/${id}`,
        );
  ...
  if (!ticket) {
    return (
      <div className="min-h-screen bg-stone-50 dark:bg-neutral-950 flex flex-col items-center justify-center gap-4 px-6 text-center">
        <Ticket
          size={36}
          className="text-stone-300 dark:text-stone-700 animate-pulse"
        />
  ...
  const transport =
    TRANSPORT_META[(ticket.transportType ?? "").toLowerCase()] ??
    TRANSPORT_META.bus;
  const Icon = transport.icon;
  const statusMeta =
    STATUS_META[ticket.verificationStatus] ?? STATUS_META.pending;
  ...
  const handleBookingSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          fallback.message || "Booking failed. Please try again.",
        );
      }
              ...
              <div className="bg-stone-50 dark:bg-neutral-800/40 rounded-xl p-4 text-sm space-y-1.5 border border-stone-100 dark:border-neutral-800/50">
                {[
                  ...
                  <div key={label} className="flex justify-between">
                    ...
                    <span className="font-medium text-stone-900 dark:text-stone-50 capitalize">
                      {value}
                    </span>
                  </div>
                ))}
            ...
            <div className="flex flex-col gap-5">
              ...
              {ticket.perks?.length > 0 && (
                <motion.div
                  ...
                  className="bg-white dark:bg-neutral-900 border border-stone-200 dark:border-neutral-800 rounded-2xl p-4 sm:p-5 shadow-sm"
                >
                  <h3 className="text-sm font-semibold text-stone-900 dark:text-stone-50 mb-3 flex items-center gap-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {ticket.perks.map((perk) => (
                      <span
                        key={perk}
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-stone-50 dark:bg-neutral-800/40 border border-stone-200 dark:border-neutral-700/70 text-sm font-medium text-stone-700 dark:text-stone-300"
                      >
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <button
                            onClick={() =>
                              setQty((q) =>
                                Math.min(ticket.quantity ?? 1, q + 1),
                              )
                            }
                            className="w-9 h-9 rounded-xl border border-stone-200 dark:border-neutral-700 flex items-center justify-center text-stone-600 dark:text-stone-300 hover:bg-stone-100 dark:hover:bg-neutral-800 transition-colors font-bold text-lg"
                          >
                ...
                {!showBook && !sessionPending && (
                  <div className="text-center py-2.5 px-3 rounded-xl bg-stone-50 dark:bg-neutral-800/50 border border-stone-200 dark:border-neutral-800">
                    <p className="text-xs text-stone-400 dark:text-stone-500 font-medium">
                      {role === "admin" ? "Admins" : "Vendors"} cannot book
                      tickets.
                    </p>
                  </div>
                )}
